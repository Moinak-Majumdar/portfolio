...
const HireMe = () => {
    ...
    const { ubuntu, roboto, poppins } = useFont();
    const darkMode = useSelector((s: RootState) => s.darkMode.mode);
    const theme = useSelector((s: RootState) => s.colorTheme);

    return (
        <motion.div initial='closed' whileInView='open' viewport={{ once: true, amount: 0.2 }} variants={outerVariants} className={`relative flex flex-col lg:flex-row lg:justify-start justify-center lg:items-start items-center p-4 lg:p-10 bg-gradient-to-b ${darkMode ? 'text-gray-300 from-[#0f0c29] via-[#302b63] to-[#24243e]' : 'text-gray-800 from-[#f2fcfe] to-[#E0EAFC]'}`}>
            <motion.div variants={Left} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className='px-1 pt-1 w-fit border relative -top-16 lg:-top-20' style={{ borderColor: hover ? theme.val : 'rgb(107,114,128)', WebkitTransition: 'all .8s ease-in-out' }}>
            ...
            <motion.div variants={Right} className='ml-0 lg:ml-24 lg:max-w-[60%] max-w-full flex flex-col'>
                <motion.h1 className={`${poppins.className} text-2xl`} style={{ color: theme.val }} initial={{ y: -5 }} animate={{ y: 5 }} transition={{ duration: .6, repeat: Infinity, repeatType: 'reverse' }}>Hello There !</motion.h1>
                ...
                <div className='flex flex-col my-2 mr-auto'>
                    <FaQuoteLeft className='mt-4 text-3xl lg:text-4xl' style={{ color: theme.val }} />
            ...
            <div className='absolute bottom-5 left-10 hidden lg:flex flex-col'>
                <AnimatedHeading classList='text-6xl 2xl:text-6xl font-bold' font={ubuntu} title='#Full Stack' color={darkMode ? '#00001180' : '#ffffff'} />
            </div>
        </motion.div>
    )
}

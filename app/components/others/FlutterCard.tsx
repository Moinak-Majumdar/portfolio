...
import { monsterRat, poppins, robotoMono } from "@/app/utils/Fonts";
    ...
    <Link href={`flutter/${info.slug}`} scroll={false} className='flex flex-col-reverse md:flex-row justify-start group rounded-xl dark:bg-slate-800 dark:shadow-black bg-gray-100 shadow-slate-400'>
      <motion.div className="p-6 flex flex-col w-full md:w-1/2" initial='closed' whileInView='open' viewport={{ once: false, amount: 0.3 }} variants={outerVariants}>
        <motion.div variants={innerVariants} className="flex items-center">
          ...
          <p style={robotoMono.style} className={`my-auto ml-4 px-2 py-1 rounded-full text-xs text-center font-semibold ${info.status == 'completed' ? 'bg-green-500 text-green-100' : 'bg-red-500 text-red-100'}`}>{info.status}</p>
        </motion.div>
        ...
        <motion.p variants={innerVariants} style={monsterRat.style} className='mt-6 text-sm font-semibold lg:mt-auto  group-hover:text-blue-500 px-4 py-2 rounded-md w-fit dark:bg-gray-700  dark:text-slate-300 bg-gray-200 text-slate-600'>Read More</motion.p>
      </motion.div>

...
const DocCard = ({ theme, darkMode, data }) => {
    ...
    return (
        <Link href={`Doc/${data.name}`}>
            <motion.a variants={innerVariants} className='w-full h-fit group overflow-hidden border p-1 border-slate-700 text-gray-300 hover:cursor-pointer rounded-tl-[40px] rounded-br-[40px]' whileTap={{scale: 0.95}}>
                <div className='flex bg-gradient-to-b from-[#243B55] to-[#141E30] relative rounded-tl-[40px] rounded-br-[40px] overflow-hidden'>
                    <div className='min-w-full h-fit lg:group-hover:opacity-0 duration-500'>
                        ...
                        <div className='absolute bottom-4 left-3 px-4 py-2 rounded-tr-2xl rounded-bl-2xl' style={{ backgroundColor: theme.val }}>
